...
TEST_F(TrackInitTest, run)
{
    ...
    initialize_tracks(states, params, initializers);
    ...
    find_vacancies(states, initializers);

    // Check the vacancies
    output_vacancies   = vacancies_test(initializers.device_pointers());
    expected_vacancies = {1, 4, 7};
    EXPECT_VEC_EQ(expected_vacancies, output_vacancies);

    // Launch a kernel to create track initializers from
    // interactions/secondaries
    process_secondaries(states, params, initializers);

    // Check the energies of the track initializers created from secondaries
    output_initializers   = initializers_test(initializers.device_pointers());
